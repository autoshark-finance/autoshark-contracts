...
contract StrategyCompoundPantherV2 is IStrategy, Ownable {
    ...
    function _depositTo(uint _amount, address _to, address _referrer) private {
        ...
        uint shares = 0;
        uint transferTax = _amount.mul(PANTHER.transferTaxRate()).mul(PANTHER.transferTaxRate()).div(10000);
        if (totalShares == 0) {
    ...
    function withdrawAll() override external {
        ...
        uint transferTax = _withdraw.mul(PANTHER.transferTaxRate()).mul(PANTHER.transferTaxRate()).div(10000);
