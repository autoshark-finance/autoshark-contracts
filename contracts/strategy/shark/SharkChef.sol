...
import "./interfaces/ISharkChef.sol";
import "../interfaces/IStrategy.sol";
import "./SharkToken.sol";


contract SharkChef is ISharkChef, OwnableUpgradeable {
    ...
    ISharkMinterV2 public minter;

    uint public startBlock = 0;
    uint public override sharkPerBlock = 40;
    uint public override totalAllocPoint;

    /* ========== MODIFIERS ========== */

    modifier onlyVaults {
        require(vaults[msg.sender].token != address(0), "SharkChef: caller is not on the vault");
        _;
    }
    ...
    modifier updateRewards(address vault) {
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint rewards = multiplier.mul(sharkPerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
                vaultInfo.accSharkPerShare = vaultInfo.accSharkPerShare.add(rewards.mul(1e12).div(tokenSupply));
            }
    ...
    event SharkRewardPaid(address indexed user, address indexed vault, uint amount);

    /* ========== INITIALIZER ========== */

    function initialize(uint _startBlock, uint _sharkPerBlock) external initializer {
        __Ownable_init();

        startBlock = _startBlock;
        sharkPerBlock = _sharkPerBlock;
    }
    ...
    function pendingShark(address vault, address user) public view override returns (uint) {
        UserInfo storage userInfo = vaultUsers[vault][user];
        VaultInfo storage vaultInfo = vaults[vault];

        uint accSharkPerShare = vaultInfo.accSharkPerShare;
        uint tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            uint multiplier = timeMultiplier(vaultInfo.lastRewardBlock, block.number);
            uint sharkRewards = multiplier.mul(sharkPerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
            accSharkPerShare = accSharkPerShare.add(sharkRewards.mul(1e12).div(tokenSupply));
        }
        return userInfo.pending.add(userInfo.balance.mul(accSharkPerShare).div(1e12).sub(userInfo.rewardPaid));
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function addVault(address vault, address token, uint allocPoint) public onlyOwner {
        require(vaults[vault].token == address(0), "SharkChef: vault is already set");
        bulkUpdateRewards();
    ...
    function updateVault(address vault, uint allocPoint) public onlyOwner {
        require(vaults[vault].token != address(0), "SharkChef: vault must be set");
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(address(minter) == address(0), "SharkChef: setMinter only once");
        minter = ISharkMinterV2(_minter);
    }

    function setSharkPerBlock(uint _sharkPerBlock) external onlyOwner {
        bulkUpdateRewards();
        sharkPerBlock = _sharkPerBlock;
    }
    ...
    function notifyDeposited(address user, uint amount) external override onlyVaults updateRewards(msg.sender) {
        ...
        uint pending = userInfo.balance.mul(vaultInfo.accSharkPerShare).div(1e12).sub(userInfo.rewardPaid);
        userInfo.pending = userInfo.pending.add(pending);
        userInfo.balance = userInfo.balance.add(amount);
        userInfo.rewardPaid = userInfo.balance.mul(vaultInfo.accSharkPerShare).div(1e12);
    ...
    function safeSharkTransfer(address user) external override onlyVaults updateRewards(msg.sender) returns (uint) {
        UserInfo storage userInfo = vaultUsers[msg.sender][user];
        VaultInfo storage vaultInfo = vaults[msg.sender];
        ...
        minter.mint(amount);
        minter.safeSharkTransfer(user, amount);
        emit SharkRewardPaid(user, msg.sender, amount);
        return amount;
    }
    ...
    function recoverToken(address _token, uint amount) virtual external onlyOwner {
        require(_token != address(SHARK), "SharkChef: cannot recover SHARK token");
